...
The most challenging part of the task was the `Flask` itself. Some may argue otherwise but I think there is no one standard way for developing an API with `Flask` which can really confuse you to the point of "Am I doing it right?". It might be held as an advantage that Flask requires libraries to run -making it as light as possible- but then again a lot of these libraries are out dated and not being developed anymore in favor of other libraries. But I guess once you grab hold of `Flask` concept, the path is easy to follow.
...
- Errors with code `404` (Not Found) and `500` (Internal Server Error) and `405` (Method Not Allowed) are converted to be displayed in json format. For `500` error case, the debug mode should be off.
